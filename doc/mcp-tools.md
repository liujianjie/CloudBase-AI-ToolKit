  ...
  parameters={[
    {
      ...
      type: "string",
      required: true,
      description: `查询类型：list=环境列表/摘要筛选（按 DescribeEnvs 语义筛选，支持通过 envId 筛选，返回 EnvId、Alias、Status、EnvType、Region、PackageId、PackageName、IsDefault），info=指定环境的详细信息（必须传入 envId，返回资源字段和计费信息），domains=安全域名列表，hosting=静态网站托管配置 可填写的值: "list", "info", "domains", "hosting"`,
    },
    {
      name: "alias",
      type: "string",
