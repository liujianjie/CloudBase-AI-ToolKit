...
当前提供约 41 个 MCP 工具，按插件分布如下。完整参数与说明见 [MCP 工具](/ai/cloudbase-ai-toolkit/mcp-tools)。
...
| functions | `queryFunctions`, `manageFunctions`, `getFunctionList`, `createFunction`, `updateFunctionCode`, `updateFunctionConfig`, `invokeFunction`, `getFunctionLogs`, `getFunctionLogDetail`, `manageFunctionTriggers`, `readFunctionLayers`, `writeFunctionLayers` |
