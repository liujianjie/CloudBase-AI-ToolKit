...
3. **Deploy functions correctly**
   - **MCP Tool**: Prefer `manageFunctions` with `action: "createFunction"` or `action: "updateFunctionCode"`
...
4. **Query logs properly**
   - Prefer `queryFunctions` with `action: "listFunctionLogs"` for log list
   - Prefer `queryFunctions` with `action: "getFunctionLogDetail"` and RequestId for detailed logs
...
Use `manageFunctions` with `action: "createFunctionAccess"` to configure HTTP access:
...
**Primary Method:** Use `manageFunctions` with `action: "createFunctionAccess"` (see MCP tool documentation).
...
**Primary Function Entrances:**
- `queryFunctions` - Query functions, logs, layers, triggers, HTTP access, and download URLs
- `manageFunctions` - Create/update/invoke functions, manage triggers, layers, and HTTP access

**Compatibility Function Tools:**
- `getFunctionList` - Compatibility alias for function list/detail
- `createFunction` - Compatibility alias for function creation
  - `type: "Event"` - Event Function (default)
  - `type: "HTTP"` - HTTP Function
  - `protocolType: "WS"` - Enable WebSocket for HTTP Function
- `updateFunctionCode` - Compatibility alias for function code update
- `updateFunctionConfig` - Compatibility alias for function configuration update

**Logging:**
- `queryFunctions(action="listFunctionLogs")` - Get function log list
...
**HTTP Access:**
- `manageFunctions(action="createFunctionAccess")` - Create HTTP access for function
- `callCloudApi` (Plan B) - Use `CreateCloudBaseGWAPI` action if direct tool unavailable

**Triggers:**
- `manageFunctions(action="createFunctionTrigger" | "deleteFunctionTrigger")` - Manage function triggers
