 ...
 * 
 * 直接使用CloudBase Manager SDK验证Event函数是否支持Python/PHP/Java/Go运行时
 * 绕过MCP工具的限制,测试SDK本身的能力
 * 
 * 运行方式:
...
describe('CloudBase SDK Runtime Validation - Direct SDK Test', () => {
  let manager = null;
  const testFunctionsDir = join(tmpdir(), `cloudbase-runtime-test-${Date.now()}`);
  const timestamp = Date.now();

  beforeAll(async () => {
    if (!hasCloudBaseCredentials()) {
      console.log('⚠️ No CloudBase credentials, skipping SDK runtime validation tests');
      return;
    }
